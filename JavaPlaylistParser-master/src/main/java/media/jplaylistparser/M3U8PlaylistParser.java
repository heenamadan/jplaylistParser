...
private final static String[] masterTags = new String[]{"#EXT-X-VERSION"};
    
    private final static String PROTOCOL = "^[H|h][T|t][T|t][P|p].*";
    private final static String[] masterSegmentTags = new String[]{"#EXT-X-STREAM-INF", "#EXT-X-I-FRAME-STREAM-INF", "#EXT-X-SESSION-DATA", "#EXT-X-SESSION-KEY"};
    private final static String[] mediaPlayListTags = new String[]{"#EXT-X-TARGETDURATION", "#EXT-X-MEDIA-SEQUENCE", "#EXT-X-DISCONTINUITY-SEQUENCE", "EXT-X-I-FRAMES-ONLY"};
    ...
    private PlayListAbstractClass  parsePlaylists(String uri, InputStream stream, PlayListAbstractClass playlist) throws IOException, JPlaylistParserException {
		...
		while ((line = reader.readLine()) != null) {
			...
			if (!(line.equalsIgnoreCase(EXTENDED_INFO_TAG) &&
				...
				if(Arrays.asList(masterSegmentTags).contains(splitted[0])){
      ...
      List<String> tagList1=new ArrayList<String>();
		...
		while ((line = reader.readLine()) != null) {
			...
			if (!(line.equalsIgnoreCase(EXTENDED_INFO_TAG) &&
				...
				}else if(Arrays.asList(masterTags).contains(splitted[0])){
					masterPlayList=new MasterPlayList();
					if(splitted[0].equalsIgnoreCase(masterTags[0])){
					masterPlayList.setVersion(splitted[1]);
					}
					listOfMasterPlayList.add(masterPlayList);
					masterMap.put(splitted[0], listOfMasterPlayList);
		...
		Set intersect = new TreeSet(Arrays.asList(masterSegmentTags));
